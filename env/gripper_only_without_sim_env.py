import numpy as np
from utils.transforms import euler_angles_to_quat, calc_trans_matrix, quat_to_rot_matrix, rot_matrix_to_quat, calc_pose_from_vel
from utils.transforms import transform_points
from env.feature import feature
from utils.vs_controller import PBVSNumpy
from scipy.spatial.transform import Rotation as R

class env:
    def __init__(self, in_hand_error, random_relative_pose) -> None:
        self.in_hand_error = in_hand_error
        self.random_relative_pose = random_relative_pose
        self.time_step = 0
        self.width = 640
        self.height = 480
        ...
        self.hole_feature_points = np.array([[0.045, 0.015, 0.03], 
                                        ...
                                        [0.045, -0.015, 0.03], 
                                        [0.015, 0.015, 0.03]])
        # self.hole_feature_points = np.array([[0.075, 0.03, 0.03], 
        #                                 [0.015, -0.03, 0.03], 
        #                                 [0.075, -0.03, 0.03], 
        #                                 [0.015, 0.03, 0.03]])
    ...
    def get_observation(self):
        ...
        vel_gt, (_, _) = self.pbvs_controller.cal_action_curve(self.cur_camera_trans_matrix, 
        ...
        hole_ref_feature, peg_ref_feature = self.feature.project_points_to_img(self.ref_camera_trans_matrix, self.hole_trans_matrix, self.hole_trans_matrix @ self.desired_trans_matrix_to_hole)
        hole_desired_feature, _ = self.feature.project_points_to_img(self.desired_camera_trans_matrix, self.hole_trans_matrix, self.hole_trans_matrix @ self.desired_trans_matrix_to_hole)
        delta_peg_trans_matrix = np.linalg.inv(self.cur_peg_trans_matrix) @ self.desired_peg_trans_matrix
        self.delta_peg_pos = delta_peg_trans_matrix[:3, 3]
        self.delta_peg_ori = R.from_matrix(delta_peg_trans_matrix[:3, :3]).as_rotvec()
        # print("delat_peg_pos: ", np.linalg.norm(delta_peg_pos))
        # print("delta_peg_ori: ", np.linalg.norm(delta_peg_ori))
        if np.linalg.norm(self.delta_peg_pos) < 0.001 and np.linalg.norm(self.delta_peg_ori) < 0.001:
            done = True
        else:
            done = False
        
        return done, self.time_step, vel_gt, hole_ref_feature, peg_ref_feature, hole_feature_to_image, peg_feature_to_image, hole_desired_feature


    def check_reset(self, done, time_step, hole_ref_feature, hole_feature_to_image, desired_hole_feature):
        info = {"success": False,
                "continue": False,
                "start pose out of range": False,
                "cur out of range": False,
                "ref out of range": False,
                "desired out of range":False,
                "time out": False}
        reset_flag = False
        if done:
            reset_flag = True
            info["success"] = True
            return reset_flag, info
        if np.max(hole_feature_to_image, axis=0)[0] > 640 or np.min(hole_feature_to_image, axis=0)[0] < 0:
            reset_flag = True
            if time_step > 0:
                info["cur out of range"] = True
            else:
                info["start pose out of range"] = True
        if np.max(hole_feature_to_image, axis=0)[1] > 480 or np.min(hole_feature_to_image, axis=0)[1] < 0:
            reset_flag = True
            if time_step > 0:
                info["cur out of range"] = True
            else:
                info["start pose out of range"] = True
        if np.max(hole_ref_feature, axis=0)[0] > 640 or np.min(hole_ref_feature, axis=0)[0] < 0:
            reset_flag = True
            info["ref out of range"] = True
        if np.max(hole_ref_feature, axis=0)[1] > 480 or np.min(hole_ref_feature, axis=0)[1] < 0:
            reset_flag = True
            info["ref out of range"] = True
        if np.max(desired_hole_feature, axis=0)[0] > 640 or np.min(desired_hole_feature, axis=0)[0] < 0:
            reset_flag = True
            info["desired out of range"] = True
        if np.max(desired_hole_feature, axis=0)[1] > 480 or np.min(desired_hole_feature, axis=0)[1] < 0:
            reset_flag = True
            info["desired out of range"] = True
        if time_step > 2000:
            reset_flag = True
    ...
    def apply_vel(self, vel, dt = 1.0 / 60.0):
        self.time_step += 1
        # vel = transform_vel(vel, self.cur_camera_trans_matrix)
    ...
    def sample(self):
        ...
        if self.in_hand_error == True:
            in_hand_pos_upper = np.array([0.01, 0.0, 0.15])
            in_hand_pos_lower = np.array([-0.01, 0.0, 0.10])
            in_hand_rot_upper = np.array([0.0, 0.8, 0.0])
            in_hand_rot_lower = np.array([0.0, 1.0, 0.0])
        else:
        ...
        init_relative_rot_lower = np.array([-np.pi/10, -np.pi/10, -np.pi/4])
        init_relative_rot_upper = np.array([np.pi/10, np.pi/10, np.pi/4])
        disturb_rot_lower = np.array([-np.pi/10, -np.pi/10, 0])
        disturb_rot_upper = np.array([np.pi/10, np.pi/10, 0])
    ...
    def sample_desired_feature(self):
        if self.random_relative_pose == True:
            desired_pos_upper = np.array([0.01, 0.01, 0.1])
            desired_pos_lower = np.array([-0.01, -0.01, 0.05])
            desired_rot_upper = np.array([np.pi/3, np.pi/3, np.pi/3])
            desired_rot_lower = np.array([-np.pi/3, -np.pi/3, -np.pi/3])
        else:
            desired_pos_upper = np.array([0.0, 0.0, 0.1])
            desired_pos_lower = np.array([0.0, 0.0, 0.1])
            desired_rot_upper = np.array([0, 0, 0])
            desired_rot_lower = np.array([0, 0, 0])

        self.desired_pos_to_hole = np.random.uniform(desired_pos_lower, desired_pos_upper)
        self.desired_pos_to_hole = self.desired_pos_to_hole + np.array([0, 0, 0.05])
        ...
        self.desired_peg_trans_matrix = self.hole_trans_matrix @ self.desired_trans_matrix_to_hole
